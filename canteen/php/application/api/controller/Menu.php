...
class Menu extends Api
{
  ...
  protected $noNeedLogin = ['usedesk', 'index', 'read'];
  ...
  public function index()
  {
    $str = "A";
    //获取类列表
    $class_list = model('MenuClass')->select();
    ...
    foreach ($class_list as $key => $value) {
      ...
      ],$MenuModel->where('class',$value['id'])->field([
        'img as picture',
        'name as `desc`',
        'id'
      ])->select());
  ...
  {
    $info = model('Menu')->find($id);
    $this->result('操作成功', $info, 1);
  }
